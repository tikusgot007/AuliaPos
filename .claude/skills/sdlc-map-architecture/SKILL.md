...
7. **Skill Execution (Mandatory Template):** You **MUST** strictly follow the procedural workflow and utilize the Mandatory Architecture Template defined in `.claude/skills/sdlc-map-architecture/references/ARCHITECTURE-TEMPLATE.md`.
...
1.  **High-Level Scan:**
    ...
    - **Prior Work Scan:** Read existing files in `docs/` and `spec/` directories to understand previously established architectures, API contracts, and business logic. Also, read any formatting rules in `.claude/standards/` if you need to generate new documentation.
...
2.  **Content:** The file's content **MUST** adhere to the Mandatory Architecture Template. You MUST read this template from `.claude/skills/sdlc-map-architecture/references/ARCHITECTURE-TEMPLATE.md` before generating the document.
...
1.  Locate the `AGENTS.md` file (or the primary agent configuration/index file in the `.claude/` or root directory).
