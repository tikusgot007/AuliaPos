...
- Boleh Selesai — **hanya jika** `status_pembayaran = lunas`, dan hanya lewat jalur yang diizinkan untuk role terkait (lihat 4.3–4.5 di bawah).
...
user yang mengubah        = admin  (atau kasir pemilik lewat jalur khusus, lihat 4.3;
                                     atau Effective Shift Leader, lihat 4.5)
```
...
| Effective Shift Leader saat itu (lihat 4.5) | Ya, jika `lunas` | Ya, jika `lunas` **dan** transaksi miliknya sendiri |
| kasir biasa | **Tidak** — selalu ditolak backend | Ya, jika `lunas` **dan** transaksi miliknya sendiri dari POS |

Syarat `lunas` identik di semua jalur; yang berbeda hanya siapa yang boleh memicu dan dari mana. Rumusan permission kasir bukan "kasir boleh mengubah status menjadi selesai", melainkan **"kasir boleh menyelesaikan transaksi lewat workflow Kasir/POS untuk transaksi miliknya yang sudah lunas."**
...
Kasir **tetap tidak boleh** menyelesaikan transaksi lewat workflow umum (Daftar/Detail Transaksi, `/api/ubah-status`) — selalu ditolak backend di sana, kecuali kasir tersebut sedang menjadi Effective Shift Leader (lihat 4.5).
...
- **Tidak** mengubah behavior `batal` di luar yang dijelaskan 4.6 (Section 2.3, 12).
...
- **Tidak** menambah role permanen baru (`users.role` tetap `enum('admin','kasir')`) — lihat 4.5 soal Effective Shift Leader.

### 4.5 Effective Shift Leader — kapabilitas SELESAI (Tahap 3, 2026-09-13)
`[AULIA §4.3]`

Sejak Tahap 3, workflow umum (Daftar/Detail Transaksi, `/api/ubah-status`) menerima **Admin ATAU Effective Shift Leader saat itu** — bukan admin-only lagi seperti 4.2/4.3 di atas. Ini mengisi peran "SPV" yang sudah diantisipasi sebelumnya (lihat versi lama 4.3: *"Role SPV belum dibuat"*) tanpa menjadikannya role permanen baru.

**Shift Leader BUKAN `users.role`.** Kolom itu tetap persis `enum('admin','kasir')`, tidak pernah bernilai `'shift_leader'`. Shift Leader adalah *effective authority* yang dihitung ON-DEMAND setiap request dari: `users.priority` (kolom permanen & unik di tabel `users`) + jadwal aktual hari itu (tabel `jadwal`, lihat `AULIA-02-modul-pendukung.md` §1) + jam shift (`JadwalModel::DEFINISI_SHIFT`, tidak diubah). Definisi lengkap konsep Priority/Shift Member/Shift Leader adalah cross-version business rule di `docs/aturan-bisnis-USER-SHIFT.md` — dokumen itu **tidak mendefinisikan ulang** konsepnya di sini, hanya mencatat titik integrasinya ke lifecycle transaksi. Satu klarifikasi penting yang menggantikan rencana awal dokumen cross-version tersebut: implementasi nyata memakai **satu Shift Leader global** untuk seluruh operasional (P/S/PM yang overlap masuk satu pool kandidat gabungan) — **bukan** satu Leader per kode shift.

**Syarat kandidat Shift Leader** (`App\Services\EffectiveShiftLeaderService`): `users.role = 'kasir'` (Admin tidak pernah ikut kompetisi ini sama sekali), `users.is_active = 1`, `users.priority IS NOT NULL`, punya row `jadwal` pada tanggal itu dengan `shift != 'L'`, dan sedang berada dalam jendela jam kerja shift tersebut (`App\Services\EvaluasiJendelaKerjaShift`). Di antara kandidat, Priority tertinggi menang. Tidak ada Leader tersimpan di database, tidak ada cache, tidak ada fallback ke Leader sebelumnya/shift lain/Admin — kalau tidak ada kandidat yang sedang bekerja, hasilnya `null`.

**Yang TIDAK berubah:** syarat `lunas` (4.2) berlaku identik untuk admin, kasir POS (4.3), maupun Shift Leader tanpa kecuali; kapabilitas kasir POS (4.3) tidak disentuh; kasir biasa yang bukan Shift Leader saat itu tetap ditolak di workflow umum; `JadwalModel::statusSaatIni()` (informational-only) tidak diubah.

Enforcement backend: `TransaksiModel::ubahStatus()` parameter `$isShiftLeader`, dihitung `Api::ubahStatus()` lewat `App\Services\Authority::isCurrentShiftLeader()` — UI (tombol "Selesai") hanya lapis pertama.

### 4.6 Effective Shift Leader — kapabilitas BATAL (Tahap 5 & 5.1, 2026-09-15)
`[AULIA §4.4]`

`SELESAI → BATAL` (sebelumnya admin-only, hanya didokumentasikan lewat komentar kode sebelum Tahap 5) diperluas jadi **Admin ATAU Effective Shift Leader saat itu**, persis pola 4.5.

**Tahap 5.1 (hari yang sama):** `PROSES → BATAL` — yang tadinya terbuka untuk **semua role** yang login — **diperketat** menyusul keputusan produk eksplisit: satu aturan tunggal berlaku untuk kedua transisi (`PROSES → BATAL` maupun `SELESAI → BATAL`): hanya Admin atau Effective Shift Leader saat itu. Kasir biasa tidak lagi bisa membatalkan transaksi apa pun lewat workflow umum.

`PROSES → MANGKRAK` **tetap murni admin-only** (TIDAK ikut diperluas ke Shift Leader — lihat Section 16). `/transaksi/batal/(:num)` (route/controller lama, dead code, tidak dipakai di view manapun) tidak disentuh, tapi karena memanggil `TransaksiModel::ubahStatus()` yang sama, gate barunya otomatis berlaku juga kalau jalur ini suatu saat dipakai lagi.
...
Enforcement backend: `TransaksiModel::ubahStatus()` — cabang `selesai → batal` dan `proses → batal` sama-sama memeriksa `$isAdmin || $isShiftLeader`. Tombol "Batalkan" sejak Tahap 5.1 di-gate visibility-nya juga (sebelumnya selalu tampil semua role) — konsisten dengan tombol "Selesai".

### 4.7 Effective Shift Leader — kapabilitas Backdate Payment (Tahap 5, 2026-09-15)
`[AULIA §4.5]`

Backdate pembayaran (fitur lengkapnya di `AULIA-CHANGELOG.md` P11) sebelumnya admin-only murni. Sekarang menerima **Admin ATAU Effective Shift Leader saat itu** — satu paket, dua hal sekaligus:

1. Mencatat pembayaran dengan `tanggal` backdate (berbeda >60 detik dari waktu server).
2. Mengoverride `kasir_id` penerima pembayaran (siapa yang benar-benar menangani uangnya).

**Validasi yang TETAP berlaku tanpa kecuali** (sama seperti syarat `lunas` di 4.5 — tidak ada yang dilonggarkan): tanggal tidak boleh di masa depan, tanggal tidak boleh sebelum tanggal transaksi. Berlaku di dua titik pemanggilan `tambahPembayaran()`: `Api::tambahPembayaran()` (workflow umum) dan `Tagihan::lunasi()` (halaman Tagihan). **Tidak berlaku** untuk pembayaran awal transaksi BARU di `kasir/index.php` — di luar scope backdate sama sekali.

Enforcement backend: `TransaksiModel::tambahPembayaran()` parameter `$isShiftLeader` baru, dihitung lewat `Authority::isCurrentShiftLeader()` di kedua controller di atas. `GET /api/kasir-list` (dropdown "Kasir Penerima") ikut terbuka utk Shift Leader, bukan admin-only lagi.

**Perubahan UI (2026-09-16):** checkbox opsional "Pembayaran diterima sebelumnya" di dalam modal Tunai/DP/Konfirmasi diganti **tombol terpisah** ("Bayar Backdate"/"Lunasi Backdate") di halaman detail transaksi — dipicu setelah insiden nyata (Shift Leader lupa mencentang checkbox, pembayaran tercatat dengan tanggal hari ini tanpa peringatan). Tombol ini membuka modal pembayaran yang sama, tapi langsung menampilkan field tanggal (wajib) & kasir penerima di modal utama SEBELUM metode dipilih. Tombol pembayaran normal tidak menampilkan field ini sama sekali — behavior identik sebelum perubahan ini. Detail lengkap di `AULIA-CHANGELOG.md` P11.
...
Siapa yang boleh membatalkan (admin/Effective Shift Leader, sejak Tahap 5.1) — lihat Section 4.6.
...
Aksi per status:
- `proses` → Edit, Selesai (admin/Effective Shift Leader di halaman ini, hanya jika `lunas`; kasir menyelesaikan transaksinya sendiri lewat workflow Kasir/POS — Section 4.3), Batal (admin/Effective Shift Leader — Section 4.6).
...
Tombol Selesai & Batalkan di Daftar/Detail **hanya ditampilkan untuk admin/Effective Shift Leader saat itu** (Section 4.5, 4.6). Kasir biasa tidak melihatnya di sini, dan backend menolaknya. Penyembunyian tombol hanyalah lapis pertama — backend selalu memvalidasi ulang role, kepemilikan, `sumber`, status pembayaran, dan status transaksi sebelum eksekusi.
...
- **Admin-only** (baik menandai maupun mengaktifkan kembali) — beda dari `BATAL` (Section 4.6) yang sejak Tahap 5.1 juga terbuka untuk Effective Shift Leader.
...
| Reminder tagihan (lihat `AULIA-02-modul-pendukung.md` §3) | Dikeluarkan |
...
> Sejak 2026-09-05: `selesai` HANYA boleh dicapai jika `lunas` DAN ditandai eksplisit. `lunas` sendirian tidak pernah cukup untuk menjadi `selesai`. Penanda eksplisit itu: **admin**, **kasir pemilik** lewat workflow Kasir/POS untuk transaksinya sendiri yang sudah lunas (Phase 2), atau — sejak Tahap 3 — **Effective Shift Leader** saat itu lewat workflow umum (Section 4.5). Syarat `lunas` tidak pernah bisa dilewati siapa pun.

> **Tagihan ditentukan berdasarkan status pembayaran, bukan status transaksi; transaksi `batal` selalu dikecualikan.**

> **Membatalkan transaksi (PROSES→BATAL maupun SELESAI→BATAL) sejak Tahap 5.1 hanya boleh oleh admin atau Effective Shift Leader saat itu** (Section 4.6) — kasir biasa tidak lagi bisa membatalkan transaksi apa pun lewat workflow umum.

> **Backdate pembayaran (tanggal berbeda dari sekarang) boleh oleh admin atau Effective Shift Leader saat itu (Section 4.7, sejak Tahap 5), tidak pernah mengubah `created_at`, dan divalidasi ulang di backend terlepas dari apa yang dikirim client.** (Lihat `AULIA-CHANGELOG.md` P11.)
