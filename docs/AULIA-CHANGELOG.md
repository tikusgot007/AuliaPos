...
- Tombol digating `session()->get('role') === 'admin'` di halaman Daftar/Detail — kasir tidak melihat tombol ini di sana (lapis 1). Backend P8 tetap jadi lapis 2 yang otoritatif. (Diperluas oleh Phase 2 — Section 4.3 `AULIA-01-transaksi-dan-kasir.md`: kasir kini bisa menyelesaikan transaksinya sendiri lewat workflow Kasir/POS, bukan lewat halaman Daftar/Detail. Diperluas lagi oleh Tahap 3 — Section 4.5: Effective Shift Leader saat itu ikut tampil di sini juga.)
...
Fitur besar: Admin (dan sejak Tahap 5, 2026-09-15, Effective Shift Leader saat itu — lihat `AULIA-01-transaksi-dan-kasir.md` §4.7) bisa mencatat pembayaran dengan **tanggal berbeda dari sekarang** (uang sudah diterima sebelumnya, baru dicatat belakangan) dan memilih **kasir penerima** yang sebenarnya menangani.

**Evolusi mekanisme UI:**
- **2026-09-05 (awal):** cukup checkbox opsional "Pembayaran diterima sebelumnya" di modal Tunai/DP/Konfirmasi yang sudah ada.
- **2026-09-16:** diganti **tombol terpisah** "Bayar Backdate"/"Lunasi Backdate" di halaman detail transaksi — dipicu insiden nyata (Shift Leader lupa mencentang checkbox, pembayaran tercatat dengan tanggal hari ini tanpa peringatan apa pun). Tombol baru langsung menampilkan field tanggal (wajib) & kasir penerima di modal utama SEBELUM metode pembayaran dipilih — bukan lagi tersembunyi di balik checkbox yang bisa terlewat. Tombol pembayaran normal ("Bayar Sekarang"/"Lunasi") tidak menampilkan field ini sama sekali, behavior-nya identik seperti sebelum perubahan ini.
...
- backdate **wajib admin atau Effective Shift Leader saat itu** (sejak Tahap 5 — sebelumnya wajib admin murni) — kasir biasa yang mencoba mengirim tanggal manual tetap ditolak backend meski lolos UI;
- tanggal tidak boleh **sebelum tanggal transaksi** (dibandingkan di level **tanggal/hari saja**, bukan jam) dan tidak boleh **di masa depan** (dibandingkan penuh sampai jam) — berlaku tanpa kecuali untuk admin maupun Shift Leader.
...
**Endpoint baru:** `GET /api/kasir-list` (admin atau Effective Shift Leader saat itu, sejak Tahap 5 — sebelumnya admin-only) untuk dropdown "Kasir Penerima". Histori pembayaran menampilkan nama kasir penerima + badge "Dicatat belakangan" (murni tampilan, tidak disimpan di database).

File (2026-09-05): `TransaksiModel.php`, `Api.php`, `Tagihan.php`, `Transaksi.php`, `Routes.php`, `components/payment/modal.php`, `payment.js`, `transaksi/index.php`, `transaksi/detail.php`, `tagihan/index.php`. File tambahan (2026-09-16, UI tombol terpisah): `transaksi/detail.php` (2 tombol baru), `components/payment/modal.php` (section tanggal/kasir dipindah ke modal utama, dihapus dari 3 modal metode), `payment.js` (state `backdateMode`, guard sebelum lanjut ke metode).

Pengujian: 17 skenario standalone lulus (15 acceptance test dokumen + 2 regresi tambahan) untuk mekanisme awal; suite PHPUnit (200+ test) tetap hijau setelah perubahan Tahap 5 dan UI 2026-09-16.
...
- Role SPV, jika suatu saat dibutuhkan selain admin/kasir — **sudah terisi sebagian** oleh Effective Shift Leader (non-permanent authority, bukan role baru; lihat `AULIA-01-transaksi-dan-kasir.md` §4.5).
