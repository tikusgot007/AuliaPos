...
Alur: Customer kirim → Gateway ekstrak referensi (bukan isi file) → simpan referensi ke buffer → CI4 simpan referensi ke `messages.media_metadata` → kasir buka `/inbox` → browser minta unduh → Gateway ambil & dekripsi dari server WhatsApp → di-stream langsung ke browser (tidak pernah disimpan ke disk di CI4 maupun Gateway).
...
>
> Seluruh Section 12 di bawah ini adalah **spesifikasi aturan bisnis yang disepakati**, BUKAN deskripsi behavior yang sudah berjalan di kode. Per dokumen sumber (2026-09-13), tidak satu pun bagian dari Unread/Read yang sudah diimplementasikan — jangan berasumsi atau membangun di atas bagian ini seolah-olah ini existing behavior. Lihat Section 17 untuk ringkasan gap.
...
- **Tidak menyentuh Gateway/WhatsApp sama sekali** — menghapus di Inbox POS **tidak menghapus chat di WhatsApp/HP** customer maupun HP toko; murni membersihkan riwayat di sisi POS.
- UI meminta konfirmasi eksplisit (nama kontak + peringatan "tidak bisa dikembalikan") sebelum eksekusi.
- Tunduk pada aturan ownership yang sama dengan kirim balasan/media (Section 8, lihat Section 13 untuk ringkasan hak akses).
- **Syarat tambahan yang disepakati:** hapus hanya boleh **Admin**, dan conversation harus berstatus **CLOSED** dulu sebelum bisa dihapus.

### Implementation Status

> **NOT YET ENFORCED**
>
> Syarat "Admin-only + wajib CLOSED" di atas **belum ditegakkan oleh implementasi kode saat ini** — kode yang berjalan mengizinkan siapa pun dengan ownership yang sesuai (Section 8) untuk menghapus, tanpa syarat status CLOSED. Ini gap terdokumentasi antara aturan disepakati dan kode berjalan (lihat juga Section 17), bukan diubah diam-diam.

---

## 17. Known Implementation Gaps

- **Unread/Read (Section 12) — disepakati, BELUM diimplementasikan sama sekali di kode.** Seluruh Section 12 adalah spesifikasi target, bukan deskripsi behavior yang sudah berjalan.
- **Delete Conversation (Section 16) — aturan "Admin-only + wajib CLOSED dulu" belum ditegakkan kode.** Implementasi saat ini mengizinkan siapa pun dengan ownership yang sesuai menghapus, tanpa syarat status CLOSED.
...
> **Media besar (audio/video/gambar/dokumen) tidak pernah disimpan permanen** di CI4 maupun Gateway — baik referensi-untuk-didekripsi-ulang (gambar/dokumen) maupun metadata-saja (audio/video).
