...
- [x] **`feature/stage-1-reliability` (worktree `C:\projects\WA-Gateway-m1`) berisi eksekusi plan resmi M1 Wave 1 Fase 1–3**: 21 commit di atas `e18f716` (8 pra-plan `3fd5f40`..`091fe19` + 13 plan `baf1896`..`065f683`) — itulah kode yang **berjalan di folder live**; branch-nya kini `fb585f1` (+9 komit refactor pasca `/sdlc-code-review`, **belum di-deploy**) dan sudah di-push (`= origin`). `master` lokal 21 komit di depan `origin/master`; PR belum dibuat. Kode ad-hoc sandbox sudah ditimpa sesuai kontrak plan (retry 50/200/800 ms + overflow 500 event, timeout LID 2 detik + cache negatif 60 detik, dst.); E-01 dan E-06 diimplementasikan ulang sesuai plan (`ownSentRegistry`, isolasi error tanpa pesan minimal). Bukti AC-002..AC-018 simulasi; **AC-001 nyata** 23 Sep. Rincian: `docs/decisions/2026-09-21-m1-wave1-eksekusi-fase1-3.md`. Riwayat kerja ad-hoc dan konfliknya dengan plan: `docs/decisions/2026-09-21-handoff-m1-wave1-eksekusi-lokal.md`
...
- [x] 03. Durable buffer — kode + simulasi selesai (TASK-002 s/d TASK-006: retry 50/200/800 ms, overflow 500 event, `PRAGMA quick_check` SQLite); bukti **simulasi**; di-merge ke `master` **lokal di folder live** 23 Sep dan berjalan live (`065f683`)
- [x] 03. Durable buffer — verifikasi nyata (TASK-017, AC-001): **lulus 3/3, 30/30 pesan, 0 hilang, 0 duplikat** (23 Sep); merge ke `master` **lokal folder live** selesai lewat deploy TASK-019 (fast-forward, rollback point `e18f716`; `origin/master` masih `e18f716`; PR belum dibuat)
...
- [~] 16. Merge — fast-forward **lokal** di folder live 23 Sep (`e18f716`→`065f683` + `pm2 restart`); `origin/master` masih `e18f716` dan PR belum dibuat
...
- [x] 6. **M1 Gelombang 1 (Ticket 02-04) — SELESAI & LIVE 23 Sep** — eksekusi lewat `plan/plan-process-m1-wave1-incoming-reliability-v1.0.md` (plan resmi, `/sdlc-write-code`, worktree `C:\projects\WA-Gateway-m1`), **BUKAN** melanjutkan kerja ad-hoc sandbox. Baca dulu `docs/decisions/2026-09-21-handoff-m1-wave1-eksekusi-lokal.md` untuk konteks lengkap (kerja ad-hoc E-03/E-04/E-05/E-09 yang sudah ter-merge ke `feature/stage-1-reliability` **sudah ditulis ulang/disesuaikan (lihat decision log 21 Sep malam)** sesuai TASK-001/002-004/013/015 plan resmi, bukan dipakai apa adanya). Langkah menggantung:
  ...
  - [ ] Buat PR (`feature/stage-1-reliability` → `master`) — **belum dibuat**. `/sdlc-code-review` M1 **sudah dijalankan 23 Sep** (laporan `docs/audit/code-review-m1-wave1-2026-09-23.md`, verdict **0 P0 / 1 P1 / 14 P2**, spec 19/19 REQ); plan perbaikannya `plan/plan-refactor-m1-wave1-incoming-reliability-v1.0.md`. Rentang PR kini **30 komit** (`origin/master` `e18f716` → `feature/stage-1-reliability` `fb585f1` = 21 M1 + 9 refactor, final) yang **belum di-deploy dan belum diverifikasi ulang di Gateway live**
