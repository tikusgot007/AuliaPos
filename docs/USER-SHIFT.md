...
**Cross-module/cross-version:** dokumen ini satu-satunya sumber kebenaran untuk konsep Priority/Shift Member/Shift Leader. Dokumen modul lain (`AULIA.md`, dan `CHAT.md` di branch `v2.2`) tidak mendefinisikan ulang konsep ini secara lokal — cukup merujuk balik ke sini dan mendaftarkan kapabilitas spesifik yang diberikan (Section 5).
...
Dokumen ini berlaku untuk AuliaPos inti (`AULIA.md`) dan, kalau/ketika dibutuhkan, modul Chat/Inbox (hanya ada di branch `v2.2`, `CHAT.md`) serta modul operasional lain yang butuh authority bertingkat — dokumen modul lain tidak boleh mendeklarasikan aturan Priority/Shift Leader versi sendiri, cukup merujuk balik ke sini.

---

## 7. Known Implementation Gaps

- **Chat/Inbox belum memakai konsep ini sama sekali.** `CHAT.md` (branch `v2.2`) mendaftarkan "kewenangan spesifik Shift Leader untuk Chat" sebagai item belum dibahas/backlog (lihat `CHAT.md` §17) — tidak ada kode Chat yang membaca `Authority::isCurrentShiftLeader()` per dokumen ini ditulis.
