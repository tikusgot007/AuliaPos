...
>
> See [`../USER-SHIFT.md`](../USER-SHIFT.md) for current rules.
>
> Dokumen ini menyatakan "dalam satu shift hanya ada satu Shift Leader"
> (satu Leader **per kode shift**, lihat Section 5 butir 10 & 15 di
> bawah) — rencana ini **belum pernah diimplementasikan**. Implementasi
> nyata (`App\Services\EffectiveShiftLeaderService`,
> `Authority::isCurrentShiftLeader()`) memakai aturan **berbeda**: satu
> Shift Leader **global**, bukan per kode shift — lihat `../USER-SHIFT.md`
> §4 untuk definisi yang berlaku sekarang. Bagian Priority
> (`users.priority`, permanen/unik) di dokumen ini tetap konsisten
