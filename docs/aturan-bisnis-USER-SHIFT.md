...
> **STATUS: DIGANTIKAN untuk definisi Shift Leader (2026-09-16).**
> Dokumen ini menyatakan "dalam satu shift hanya ada satu Shift
> Leader" (satu Leader **per kode shift**, lihat Section 5 butir 10 &
> 15 di bawah) — tapi ini murni rencana yang **belum pernah
> diimplementasikan** di branch mana pun sebelum v2.1. Implementasi
> nyata di `v2.1` (`App\Services\EffectiveShiftLeaderService`,
> `App\Services\Authority::isCurrentShiftLeader()`) memakai aturan
> **berbeda**: **satu Shift Leader global** untuk seluruh operasional
> (kandidat P/S/PM yang overlap masuk satu pool gabungan, bukan satu
> Leader per kode shift) — lihat `docs/aturan-bisnis-AULIA.md` Section
> 4.3 untuk definisi yang benar-benar berlaku. Isi dokumen ini di
> bawah dipertahankan apa adanya sebagai catatan sejarah/konteks
> desain awal, BUKAN sebagai aturan yang berlaku sekarang untuk Shift
> Leader. Bagian Priority (kolom `users.priority`, permanen/unik) di
> dokumen ini tetap konsisten dengan implementasi `v2.1`.

> **Status asli: DISEPAKATI, BELUM DIIMPLEMENTASIKAN** (dokumen dibuat
