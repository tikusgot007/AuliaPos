...
| Pola cocok | `LIKE '%q%'`; metakarakter LIKE di-escape sekali oleh pemanggil (`strtr()`), kutip string SQL sekali oleh bind engine → `%`/`_` diperlakukan sebagai teks biasa. **Dikoreksi 2026-09-25** — semula `escapeLikeString()`, yang meng-escape dua kali; lihat `docs/walkthrough-m3-fase1e-remediation-2026-09-25.md` | CL-008, AC-014h |
...
> [!NOTE]
> **Batas pengamatan plan (e).** Separuh klaim "tick yang jatuh saat putaran masih berjalan tidak memulai request baru" tidak dapat direkam secara live: satu putaran hanya 122-327 ms pada 10 conversation live, dan bahkan dataset 200.000 pesan memakan ± 1,0 s (TASK-027) — untuk melewati tick 6 detik dibutuhkan sekitar 1,2 juta pesan. Karena itu separuh tersebut dibuktikan secara struktural: early return `if (putaranDaftarBerjalan) return;` di dalam `setInterval` 6 detik dan pelepasan penanda di `.finally()` (`app/Views/inbox/index.php:2590-2593` dan `1016-1075`), yang dikunci oleh test `tests/session/OperationalInboxScreenTest.php` dari TASK-025. Tidak ada `AbortController`, timeout, atau queue yang ditambahkan (CL-020).
>
> **Dikoreksi 2026-09-25 (remediasi `[CORR-01]`).** Kalimat "pelepasan penanda di `.finally()`" di atas menggambarkan kode **saat Fase 1e ditutup**, ketika pelepasan itu dilakukan tanpa syarat oleh putaran mana pun. Setelah remediasi Fase 2, `.finally()` hanya melepas penanda bila putaran yang selesai masih yang terbaru. Skenario putaran-yang-dikalahkan dan cara mengujinya di browser ada di `docs/walkthrough-m3-fase1e-remediation-2026-09-25.md` §4.
>
