...
**This is not theoretical in the live data.** A read-only audit of `aulia_inboxdb.messages` (2026-09-25, first pass) shows 113 rows but only 109 distinct `(conversation_id, message_timestamp)` pairs, i.e. **4 real collisions** (re-verified later the same day, after the gateway connection was restored: 121 rows / 117 pairs, the same 4 collisions):
...
**Observed, not merely predicted.** On 2026-09-25 a gateway connectivity failure was fixed at ~13:56 and the buffered backlog was flushed into the Inbox in one burst: **62 rows** (`messages.id` 204-265) across **10 conversations** inside 96 seconds (`created_at` 13:56:06-13:58:03), each carrying a `message_timestamp` between 9 and 326 minutes old. That single flush is what produced **all four collisions** in the table above (for example conversation 11745 received 6 of its flushed rows within 4 seconds, creating the ties at `08:31:33` and `08:31:35`), and it left **7 rows whose `message_timestamp` is earlier than a row inserted before them** in the same conversation. Ties and inversions are therefore a routine consequence of a real backlog flush, not a corner case invented for this plan.
