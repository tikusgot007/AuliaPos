...
- and two semantics questions that must be answered by `/sdlc-clarify-reqs` before any code changes
  (`CR-03` initiator gate reading, `CR-04` expected-owner consistency).

Normative upstream documents: `plan/plan-feature-m3-operational-inbox-fase2a-v1.0.md` v1.0 (wins on conflict,
RISK-01) and `docs/audit/clarification-report-m3-fase2a-plan-2026-09-23.md` (Q1–Q9).
...
- **REQ-007 (CR-03, clarification-gated):** align the `belum_diambil` initiator exception with `queue_status === 'belum_diambil'`
  instead of `assigned_to IS NULL`, or document the widening as intended.
- **REQ-008 (CR-04, clarification-gated):** remove the client `expected_owner` vs server-read `assigned_to` mismatch
  (fail-fast 409) so `from_user_id` can never be recorded from a stale read.
...
| TASK-102 | `tests/session/InboxHandoffTest.php`: `E10` — `unset($payload['expected_owner'])` → 400 with message `'Field expected_owner wajib dikirim.'`, ownership/history untouched; plus a negative control (same payload WITH the field → 200) so the 400 is proven to come from the missing field. | REQ-002 | [ ] | |
...
| TASK-104 | `app/Controllers/Inbox.php` (lines 993-998): add `'current_owner_id' => $assignedTo` to the `selesai` 409 body (nullable). Test: extend `H02` (or add `H02b`) to assert `current_owner_id` equals the current owner and that ownership/history are unchanged. | REQ-004 | [ ] | |
...
| TASK-108 | `app/Controllers/Inbox.php`: ekstrak blok 409 kalah (1144-1159) ke private `balas409KepemilikanBasi()` dengan payload `status`/`message`/`current_owner_id` identik, termasuk fallback `User #{id}`. Perilaku harus sama: `C01`/`C02`/`C04`/`E06` tetap hijau. Prasyarat TASK-201; jangan ekstrak hal lain. | PRN-002, CON-002 | [ ] | |
...
### Implementation Phase 2: Clarification-Gated Semantics (blocked until `/sdlc-clarify-reqs`)

- **GOAL-002:** resolve CR-03 and CR-04 according to the clarified product decision, without breaking any locked test.

| Task ID | Description (Include Exact File Paths & Micro-Testing) | Ref ID | Completed | Date |
| --- | --- | --- | --- | --- |
| TASK-201 | Only after the clarification: `app/Controllers/Inbox.php` (around lines 1088-1104) — fail-fast `409` when the normalised `expected_owner` differs from the server-read `assigned_to`, reusing `balas409KepemilikanBasi()` from TASK-108. Tests: existing `C01`/`C02`/`C04`/`E06` must stay green; add one test proving no ownership write happened on that path. | REQ-008, CON-003 | [ ] | |
| TASK-202 | Option A for CR-03: change the initiator gate (`app/Controllers/Inbox.php` lines 1092-1094) to key off `queue_status === 'belum_diambil'` and mirror it in `app/Views/inbox/index.php` (lines 859-862). Test: `assigned_to IS NULL` + active `snoozed_until` (tab `ditunda`) + non-assignee kasir → 403. | REQ-007 | [ ] | |
| TASK-203 | Option B untuk CR-03 (hanya bila widening dipertahankan): tanpa perubahan kode; catat tafsir di `spec/spec-design-m3-operational-inbox-fase2a-handoff-collision.md` + docblock `handoffPercakapan()`, dan tambahkan test yang mendokumentasikan perilaku saat ini. Jangan dijalankan bersama TASK-202. | REQ-007, CON-003 | [ ] | |
...
- **FILE-001:** `app/Controllers/Inbox.php` — TASK-101 (guard tipe), TASK-103 (`mb_strlen`), TASK-104 (`current_owner_id`),
  TASK-107 (baris kosong), TASK-108 (helper 409), plus TASK-201/TASK-202 setelah klarifikasi.
- **FILE-002:** `tests/session/InboxHandoffTest.php` — TASK-101..TASK-105 (+TASK-201..TASK-203).
- **FILE-003:** `docs/ARCHITECTURE.md` — TASK-106 (menamai `UserModel::daftarKasirAktif()`).
- **FILE-004:** `app/Views/inbox/index.php` — hanya bila TASK-202 (Option A) dipilih.
- **FILE-005:** `spec/spec-design-m3-operational-inbox-fase2a-handoff-collision.md` — hanya bila TASK-203 (Option B) dipilih.
...
- **TEST-006 (REQ-008, Fase 2):** jalur fail-fast 409 tidak menjalankan write (ownership/`updated_at` tidak berubah);
  regresi `C01`/`C02`/`C04`/`E06` tetap hijau. [TASK-201]
- **TEST-007 (REQ-007, Fase 2):** `assigned_to NULL` + snooze aktif + non-assignee → 403 (Option A), atau test dokumentasi
  perilaku widening (Option B). [TASK-202 / TASK-203]
...
- **RISK-002 (REQ-008, fail-fast 409):** jalur kalah kini menjawab sebelum transaksi dijalankan. Mitigasi: status HTTP
  identik, `C01`/`C02`/`C04`/`E06` wajib tetap hijau, ditambah assertion "tidak ada write" (TEST-006).
- **RISK-003 (TASK-108, ekstraksi helper):** refactor pada method baru bisa mengubah perilaku bila tidak murni.
  Mitigasi: hanya memindahkan blok 1144-1159 apa adanya, tanpa mengubah urutan/teks pesan, dan diverifikasi suite penuh.
- **RISK-004 (TASK-202, gerbang inisiator):** mempersempit izin yang saat ini berlaku. Mitigasi: **jangan eksekusi sebelum
  `/sdlc-clarify-reqs`** (CON-003); bila dijalankan, UI (`index.php` 859-862) harus diubah di commit yang sama agar tidak
  menawarkan aksi yang pasti 403.
...
> **Handoff:** setelah Phase 1 (TASK-101..TASK-110) disetujui, lanjutkan Phase 2 hanya bila `/sdlc-clarify-reqs`
> sudah menjawab CR-03/CR-04. Implementasi wajib lewat `/sdlc-write-code` — reviewer tidak menulis source code.
