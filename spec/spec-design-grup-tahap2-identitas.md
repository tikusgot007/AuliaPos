...
**Edge case (fallback tampilan aman non-JID)**: `sender_jid` berformat di luar `@s.whatsapp.net`/`@lid` (mis. `@hosted`, `@hosted.lid`, `@g.us`, atau nilai malformed) **tidak pernah** dirender mentah sebagai JID (invarian proyek: jangan membocorkan JID mentah ke UI). Aturan tampilan untuk `REQ-008`: `@s.whatsapp.net` → nomor telepon; `@lid` → `LID`; domain lain berakhiran `.lid` → `LID`; selain itu → label generik **`Pengirim`**. Guard `400` (`REQ-010`/`AC-009`) tetap lenient (cukup non-empty) sehingga format tak terduga tidak menghentikan alur, tetapi nilainya tidak pernah ditampilkan apa adanya. Ini **tidak mengubah** `REQ-008`/`AC-002` (identitas normal tetap nomor/`LID`); ini hanya menetapkan tampilan aman untuk input tak terduga (resolusi Clarification Report, item *raw-JID fallback* **CLOSED** untuk sisi spec).
