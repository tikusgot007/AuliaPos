...
- `page` (opsional): nomor halaman, mulai dari `1`. Jika tidak diisi, gunakan `page=1`. Nilai `page` yang bukan bilangan bulat positif (termasuk `0` atau negatif) wajib menghasilkan HTTP 400. Jika `page` valid tetapi melewati halaman terakhir, response tetap HTTP 200 dengan array kosong `[]`.
